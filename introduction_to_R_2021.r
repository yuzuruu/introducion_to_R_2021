...
tp_01_tidy <- 
  ...
  # Rで使いやすい縦長データに変換する関数
  tidyr::pivot_longer(
    cols = # 縦長データに変換するデータを選ぶ。変換しないデータは選ばない。
      c(
        "total", 
        "female", 
        "male"
      ),
    names_to = "gender",　#　もともとの変数名（列名）をまとめるときに使う変数名
    values_to = "population"　# もともとの変数名が表現する内容
  ) %>% 
  # dplyr::mutate()関数
...
tp_01_lineplot_01 <- 
  ...
  # 見た目詳細を変更する
  theme(
    legend.position = c(0.7, 0.2),　# 凡例位置を調整。値はx軸y軸各々について原点からの距離。
